...
<div class="pageTitle">
	<div class="thetop">
		<a href="{{url('admin/user/create')}}"><i class="fa fa-fw fa-plus"></i>新建用户</a>
		<a href="javascript:showDialog()" class="xuan"><i class="fa fa-fw fa-sliders"></i>筛选用户</a>
	</div>
</div>
...
<div class="pageButton"></div>
  <table>
	...
	@foreach($data as $v)
			...
			<td>{{$v->email}}</td>
             @if($v->state==0)
              <td><i class="fa fa-fw fa-circle" style="color: #15db81;"></i></td>
                @else
           ...
           @endif
             <td>{{date('Y-m-d',$v->user_time)}}</td>
      	    <td class='delete'>
                @if($v->state==0)
                <a href="{{url('admin/user/close/'.$v->user_id)}}"><input type='button' name='close' id='{{$v->user_id}}' value='禁用' class="anniu"></a>
                @else
                <a href="{{url('admin/user/open/'.$v->user_id)}}"><input type='button' name='open' id='{{$v->user_id}}' value='开启' class="anniu"></a>
                @endif
                <a href="javascript:;" onclick="delArt({{$v->user_id}})"><input type='button' name='del' id='{{$v->user_id}}' value='删除'class="anniu"></a>
                <a href="{{url('admin/user/'.$v->user_id.'/edit')}}"><input type='button' name='edit' id='{{$v->user_id}}' value='修改信息' class="anniu"></a>
			 </td>
      		</tr>
	@endforeach
...
<div class="ui-dialog" id="dialogMove" onselectstart='return false;'>
    ...
    <div class="ui-dialog-content">
        ...
        <div class="ui-dialog-l40 ui-dialog-pt15">
            <input class="ui-dialog-input ui-dialog-input-password" name="email" type="input" placeholder="邮箱" />
        </div>
